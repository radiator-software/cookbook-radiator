...
radiator_installation 'radiator-archive' do
  provider 'archive'

  # Add version so the same tests work for evaluation and archive
  # The tests check for versioned directories
  version node['test']['radiator']['archive_install_version']

  # Grab source from test attribute
  install_options do
    path node['test']['radiator']['archive_source']
  end
end
